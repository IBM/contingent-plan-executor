...
def new_conversation():
    try:
        ...
        # with open("plan_data.json", "r") as plan_data: # for local runs
        #     plan_json_config = json.load(plan_data)
        #     plan_id = plan_json_config['hovor_config']['name'] 
...
def new_message():
    # TODO: check if rasa server is running yet; run it if not
    try:
        # We assume that we only have one trace per user
        input_data = request.get_json()
        user_id = input_data['user']
    ...
    if last_execution_result.get_field('type') == 'message':
        ...
        return jsonify({'status': 'success',
                        ...
                        'stickiness': 1,
                        'msg': last_execution_result.get_field('msg'),
                        # TODO: fix diagnostics not being converted to JSON correctly
                        #'diagnostics': diagnostics
                        }
                    )
    else:
