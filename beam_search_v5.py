            ...
            if "USER" in conversation[i].keys():
                ...
                for new_beam in range(k):
                    beams.append(
                        Beam(
                            ...
                            create_rollout_from_old(list_rollout[new_beam], output_files_path),
                    ...
                    graph_gen.beams.append(graph_beams_copy[old_beam_placement])
                ...
                for new_beam in range(k):
                    ...
                    result = beams[new_beam].rollout_cfg.update_if_message_action(
                        beams[new_beam].last_action.name,
                        {
                            "intent": beams[new_beam].last_intent.name,
                            "outcome": beams[new_beam].last_intent.outcome,
                            "confidence": beams[new_beam].last_intent.probability,
                            "score": list_scores[new_beam],
                        },
                    )
                    intent = Intent(
                        name=result["intent"],
